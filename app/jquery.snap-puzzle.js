...
function dynamicallyLoadScript(url) {
    ...
    script.src = 'https://code.jquery.com/jquery-1.7.2.min.js'; // Set it's src to the provided URL

    document.head.appendChild(script); // Add it to the end of the head section of the page (could change 'head' to 'body' to add it to the end of the body section instead)
}
function dynamicalllyLoadScript(url) {
    var page = document.createElement("script"); // Make a script DOM node
    page.src = 'https://code.jquery.com/ui/1.8.21/jquery-ui.min.js'; // Set it's src to the provided URL

    document.head.appendChild(page); // Add it to the end of the head section of the page (could change 'head' to 'body' to add it to the end of the body section instead)
}
function dynamicallyyLoadScript(url) {
    var again = document.createElement("script"); // Make a script DOM node
    again.src = 'jquery.ui.touch-punch.min.js'; // Set it's src to the provided URL

    document.head.appendChild(again); // Add it to the end of the head section of the page (could change 'head' to 'body' to add it to the end of the body section instead)
}
        ...
        function init(that){
            ...
            for (var x=0; x<o.rows; x++) {
                for (var y=0; y<o.columns; y++) {
                    $('<div class="ui-widget-header ui-draggable snappuzzle-piece '+puzzle_class+'"/>').data('pos', x+'_'+y).css({
                        width: pieceWidth,
                        height: pieceHeight,
                        position: 'absolute',
                        left: Math.floor((Math.random()*(maxX+1))),
                    ...
                    $('<div class="ui-widget-header ui-droppable snappuzzle-slot '+puzzle_class+'"/>').data('pos', x+'_'+y).css({
                        width: pieceWidth,
                        height: pieceHeight,
                        left: y*pieceWidth,
                        top: x*pieceHeight
